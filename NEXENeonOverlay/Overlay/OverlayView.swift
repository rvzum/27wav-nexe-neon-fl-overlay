...
//  NEXENeonOverlay
//
//  The full-window SwiftUI composition rendered inside one OverlayWindow —
//  one instance of this view per tracked FL Studio window (see
//  OverlayManager). Entirely decorative: the whole view stack sits behind
...
//
//  Traces a neon frame just inside that window's own edge (NeonFrameView),
//  a soft ambient glow pooling in its corners (AmbientLightView), and
//  minimal HUD-style corner accents — driven purely by that window's AX
//  geometry, never by reading its pixel content.
//
